...
# Load output data
load(system.file("unitTests/output.misc.rda", package = "eTTR"))
...
test.CLV <- function() {
  ...
  it <- iTop[, hlc]
  rownames(it) <- NULL
  oa <- as.data.frame(output$allCLV)
  rownames(oa) <- rownames(ia)
  ot <- as.data.frame(output$topCLV)
  rownames(ot) <- rownames(it)
